...
"""def square(number):
	if type(number) == type(8) or type(number)==type(8.0):
		return number**2
	else:
		return None
...
"""class Car:
	definition = "vehicle with four wheels and an engine"
...
def addtofront(itlist, itval):
	itlist.insert(0, itval)
...
def appendtotuple(thetuple, value):
	global shoelist
	shoelist = list(thetuple)
	shoelist.append(value)
	tuple(shoelist)
	return shoelist
	
appendtotuple(shoes, "whatever")
